...
const Card = ({ current }) => {
  ...
  return (
    <div
      onMouseOver={e => e.target && (setHover('flex'), setHoverBlock('block smq:hidden'))}
      onMouseOut={e => e.target && (setHover('hidden'), setHoverBlock('hidden'))}
      className="text-gray-200 relative rounded-2xl items-stretch col-span-1 row-span-3 hover:bg-opacity-70 smq:text-smq"
    >
      ...
      {(current.description || current.categories)
        && <div className={`${isHover} smb:text-smq absolute z-10 bg-gray-900 hover:bg-opacity-80 rounded-2xl h-[100%] items-center`}>
